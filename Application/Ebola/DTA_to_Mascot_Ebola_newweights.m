...
for utn = 1 : 5
    ...
    for tr = 1 : 1    
        ...
        flog = ['Ebola_glm_mascot_rep' num2str(i)];
...
%         fprintf(g,'\t\t\t\t</maxRate>\n');
        ...
        fprintf(g,'\t\t\t\t\t<dynamics spec="GLM" id="constant" typeTrait="@typeTraitSet.t" maxRate="20" types="Bo Bombali Bonthe Kailahun Kambia Kenema Koinadugu Kono Moyamba PortLoko Pujehun Tonkolili WesternRural WesternUrban">\n');
...
%         fprintf(g,'\t\t<operator id="KappaScaler.s" spec="ScaleOperator" parameter="@kappa.s" scaleFactor="0.5" weight="1"/>\n');
        ...
        fprintf(g,'\t\t<operator id="RootScaler.t" spec="ScaleOperator" tree="@tree" scaleFactor="0.9" optimise="false" weight="1.0"/>\n');
        fprintf(g,'\t\t<operator id="Uniform.t" spec="Uniform" tree="@tree" weight="5.0"/>\n');
        fprintf(g,'\t\t<operator id="SubtreeSlide.t" spec="SubtreeSlide" tree="@tree" size="0.05" weight="5.0"/>\n');
        fprintf(g,'\t\t<operator id="Narrow.t" spec="Exchange" tree="@tree" weight="5.0"/>\n');
        fprintf(g,'\t\t<operator id="Wide.t" spec="Exchange" tree="@tree" isNarrow="false" weight="15.0"/>\n');
        fprintf(g,'\t\t<operator id="WilsonBalding.t" spec="WilsonBalding" tree="@tree" weight="15.0"/>\n');
...
%         fprintf(g,'\t\t</operator>\n');
        ...
        fprintf(g,'\t\t<operator id="NeClockScaler" spec="ScaleOperator" scaleFactor="0.8" optimise="true" parameter="@NeClock" scaleAll="true" scaleAllIndependently="true" weight="1.0"/>\n');
		fprintf(g,'\t\t<operator id="MigrationClockScaler" spec="ScaleOperator" scaleFactor="0.8" optimise="true" parameter="@migrationClock" scaleAll="true" scaleAllIndependently="true" weight="1.0"/>\n');

		fprintf(g,'\t\t<operator id="NeScalerScaler" spec="beast.mascot.operators.RealRandomWalkOperator" windowSizeOn="0.02" windowSizeOff="0.5" indicator="@NeIndicator" parameter="@NeScaler" useGaussian="true" weight="5.0"/>\n');
        fprintf(g,'\t\t<operator id="MigrationScalerScaler" spec="beast.mascot.operators.RealRandomWalkOperator" windowSizeOn="0.02" windowSizeOff="0.5" indicator="@migrationIndicator" parameter="@migrationScaler" useGaussian="true" weight="5.0"/>\n');
        
		fprintf(g,'\t\t<operator id="indicatorFlip.s:Migration" spec="BitFlipOperator" parameter="@migrationIndicator" weight="5"/>\n');
% 		fprintf(g,'\t\t<operator id="BSSVSoperator.c:Migration" spec="BitFlipBSSVSOperator" indicator="@migrationIndicator" mu="@migrationClock" weight="30"/>\n');
		fprintf(g,'\t\t<operator id="indicatorFlip.s:Ne" spec="BitFlipOperator" parameter="@NeIndicator" weight="5"/>\n');
% 		fprintf(g,'\t\t<operator id="BSSVSoperator.c:Ne" spec="BitFlipBSSVSOperator" indicator="@NeIndicator" mu="@NeClock" weight="30"/>\n');

%         fprintf(g,'\t\t<operator id="ErrorScaler.c:Ne" spec="DeltaExchangeOperator" delta="0.05" parameter="@NeError" weight="30"/>\n');
% 		fprintf(g,'\t\t<operator id="ErrorScaler.c:Migration" spec="DeltaExchangeOperator" delta="0.05" parameter="@migrationError" weight="30"/>\n');
