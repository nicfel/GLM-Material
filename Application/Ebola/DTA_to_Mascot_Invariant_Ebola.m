...
%%
system('rm -r xmls/invariant');
system('mkdir xmls/invariant');
...
for utn = 1 : 5
    ...
    for tr = 1 : 1    
        ...
        flog = ['Ebola_glm_invariant_rep' num2str(i)];
        fname = sprintf('xmls/invariant/%s.xml',flog);
        g = fopen(fname,'w');
...
%         fprintf(g,'\t\t\t\t</maxRate>\n');
        ...
        fprintf(g,'\t\t\t\t\t</x>\n');
        fprintf(g,'\t\t\t\t\t<distr spec="beast.math.distributions.Poisson" lambda=''5''/>\n');
        fprintf(g,'\t\t\t\t</prior>\n');
...
%         fprintf(g,'\t\t<operator id="KappaScaler.s" spec="ScaleOperator" parameter="@kappa.s" scaleFactor="0.5" weight="1"/>\n');
        fprintf(g,'\t\t<operator id="TreeScaler.t" spec="ScaleOperator" tree="@tree" weight="1.0"/>\n');
        fprintf(g,'\t\t<operator id="RootScaler.t" spec="ScaleOperator" tree="@tree" weight="1.0"/>\n');
        fprintf(g,'\t\t<operator id="Uniform.t" spec="Uniform" tree="@tree" weight="30.0"/>\n');
        fprintf(g,'\t\t<operator id="SubtreeSlide.t" spec="SubtreeSlide" tree="@tree" weight="15.0"/>\n');
        fprintf(g,'\t\t<operator id="Narrow.t" spec="Exchange" tree="@tree" weight="15.0"/>\n');
        fprintf(g,'\t\t<operator id="WilsonBalding.t" spec="WilsonBalding" tree="@tree" weight="3.0"/>\n');
        fprintf(g,'\t\t<operator id="NeMigUpDownOperator.t" spec="UpDownOperator" scaleFactor="0.8" weight="1.5">\n');
        ...
        fprintf(g,'\t\t<operator id="NeClockScaler" spec="ScaleOperator" scaleFactor="0.8" optimise="true" parameter="@NeClock" scaleAll="true" scaleAllIndependently="true" weight="5.0"/>\n');
		fprintf(g,'\t\t<operator id="MigrationClockScaler" spec="ScaleOperator" scaleFactor="0.8" optimise="true" parameter="@migrationClock" scaleAll="true" scaleAllIndependently="true" weight="5.0"/>\n');

		fprintf(g,'\t\t<operator id="NeScalerScaler" spec="beast.mascot.operators.RealRandomWalkOperator" windowSizeOn="0.1" windowSizeOff="0.5" indicator="@NeIndicator" parameter="@NeScaler" useGaussian="true" weight="30.0"/>\n');
        fprintf(g,'\t\t<operator id="MigrationScalerScaler" spec="beast.mascot.operators.RealRandomWalkOperator" windowSizeOn="0.1" windowSizeOff="0.5" indicator="@migrationIndicator" parameter="@migrationScaler" useGaussian="true" weight="30.0"/>\n');
