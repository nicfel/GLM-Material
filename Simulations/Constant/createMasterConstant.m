...
if exist('mCovariates.txt')>1
    error('Covariates files already exist, if you want to redo the analysis, comment out this line');
end
...
fprintf(Ne_params, '\tNeGLMscaler.Ne_co%d',nr_covariates+1);
fprintf(m_params, '\tmigrationGLMscaler.m_co%d',nr_covariates+1);
fprintf(m_params, '\tmigrationGLMscaler.m_co%d',nr_covariates+2);
fprintf(Ne_params, '\n');
fprintf(m_params, '\n');
...
while S <= nr_runs
    ...
    active_migration = use_covariates;% poissrnd(2) + 1;
    active_Ne = use_covariates;%poissrnd(2) + 1;
    ...
    sample_nr = sample_nr*50;
    
    % add this to the Ne predictors
    log_Ne(:,end+1) = (log(sample_nr)-mean(log(sample_nr)))/std(log(sample_nr));
    scaler_Ne(end+1) = 0;
    
    % add sampling to the migration rate predictors
    counter = 1;
    for a = 1 : states
        for b = 1 : states
            if a ~= b
                mig_from(counter) = sample_nr(a);
                mig_to(counter) = sample_nr(b);
                counter = counter+1;
            end
        end
    end
    % BEWARE, since these are backwards rate, the 11 predictor is actually
    % to and the 12th from
    
    log_migration(:,end+1) = (log(mig_from)-mean(log(mig_from)))/std(log(mig_from));
    log_migration(:,end+1) = (log(mig_to)-mean(log(mig_to)))/std(log(mig_to));
    scaler_migration(end+1) = 0;
    scaler_migration(end+1) = 0;
        ...
        for l = 1 : length(temp_lines)
            ...
            elseif  ~isempty(strfind(temp_lines{l},'insert_samples')); 
                ...
                for i = 1 : states
                    ...
                    sample_nr_in_draw = poissrnd(10);
                    next_samples = min(sample_nr_in_draw,rest_samples);
